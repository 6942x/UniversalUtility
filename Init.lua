...
local function a33()
    ...
    local a36 = {
        ...
        MousePosEnabled     = a21.MousePosEnabled,
        MousePosSaved       = a21.MousePosSaved,
...
local function a41()
    ...
    a21.MousePosEnabled     = a43.MousePosEnabled or false
    a21.MousePosSaved       = a43.MousePosSaved or { X = 960, Y = 540 }
...
do
    ...
    local w10, _ = a143(a226, "Mouse Position", 162)
    local w11, _, _, w12 = a147(w10, UDim2.new(0, 56, 0, 28), UDim2.new(1, -28, 0.5, 0), a21.MousePosEnabled, nil)

    local w13 = Instance.new("TextLabel", a226)
    w13.Size = UDim2.new(1, -20, 0, 16)
    w13.Position = UDim2.new(0, 10, 0, 198)
    w13.BackgroundTransparency = 1
    w13.Text = string.format("[Locked] F5 · %d, %d", math.floor(a21.MousePosSaved.X), math.floor(a21.MousePosSaved.Y))
    w13.Font = Enum.Font.Gotham
    w13.TextSize = 12
    w13.TextColor3 = Color3.fromRGB(150, 150, 150)
    w13.TextXAlignment = Enum.TextXAlignment.Left
    ...
    a198 = {
        ...
        MousePosToggleBtn   = w11,
        MousePosToggleState = w12,
        MousePosLabel       = w13,
    ...
    _G.UU.Threads.Click = task.spawn(function()
        while a21.ClickEnabled do
            ...
            if a21.ClickEnabled then
                local w7, w8
                if a21.MousePosEnabled then
                    local w9 = a3:GetMouseLocation()
                    w7, w8 = w9.X, w9.Y
                else
                    w7, w8 = a21.MousePosSaved.X, a21.MousePosSaved.Y
                end
                a4:SendMouseButtonEvent(w7, w8, 0, true, game, 0)
                task.wait(0.05)
                a4:SendMouseButtonEvent(w7, w8, 0, false, game, 0)
            end
        end
...
a199.AutoSpamToggleBtn.MouseButton1Click:Connect(function()
    ...
    if a21.AutoSpamEnabled then
        ...
        if a404 == Enum.KeyCode.P or a404 == a21.Keybind or a404 == Enum.KeyCode.F5 then
            a21.AutoSpamEnabled = false
            a160(a199.AutoSpamToggleState, false)
...
a203.KeybindButton.MouseButton1Click:Connect(function()
    ...
    a411 = a3.InputBegan:Connect(function(a413, a414)
        if a413.UserInputType == Enum.UserInputType.Keyboard and not a414 then
            ...
            if a413.KeyCode == Enum.KeyCode.F5 then
                a203.KeybindButton.Text  = "Current Key: "..(a22[a21.Keybind] or a21.Keybind.Name)
                a203.KeybindButton.Active = true
                a21.IsChangingKeybind = false
                a411:Disconnect()
                a33_log("Keybind: F5 is reserved", Color3.fromRGB(255, 100, 100))
                return
            end
...
local function w2()
    if w1 then w1:Disconnect(); w1 = nil end
    if a21.MousePosEnabled then
        w1 = a5.RenderStepped:Connect(function()
            local w3 = a3:GetMouseLocation()
            a21.MousePosSaved.X = w3.X
            a21.MousePosSaved.Y = w3.Y
        end)
    end
end

table.insert(_G.UU.Connections, a3.InputBegan:Connect(function(w4, w5)
    if not w5 and w4.KeyCode == Enum.KeyCode.F5 then
        a21.MousePosEnabled = not a21.MousePosEnabled
        a160(a198.MousePosToggleState, a21.MousePosEnabled)
        w2()
        a33_save_log("Mouse Position → " .. (a21.MousePosEnabled and "Tracking" or "Locked"))
    end
end))

a198.MousePosToggleBtn.MouseButton1Click:Connect(function()
    a21.MousePosEnabled = not a21.MousePosEnabled
    a160(a198.MousePosToggleState, a21.MousePosEnabled)
    w2()
    a33_save_log("Mouse Position → " .. (a21.MousePosEnabled and "Tracking" or "Locked"))
...
task.spawn(function()
    while true do
        task.wait(0.25)
        if a198.MousePosLabel and a198.MousePosLabel.Parent then
            local w6 = a21.MousePosEnabled and "Tracking" or "Locked"
            a198.MousePosLabel.Text = string.format("[%s] F5 · %d, %d", w6, math.floor(a21.MousePosSaved.X), math.floor(a21.MousePosSaved.Y))
            a198.MousePosLabel.TextColor3 = a21.MousePosEnabled and Color3.fromRGB(50, 220, 100) or Color3.fromRGB(150, 150, 150)
        end
    end
end)
...
if a483 then
    ...
    if a21.MousePosEnabled then w2() end
    ...
    a160(a199.AutoSpamToggleState, false)
    a160(a198.MousePosToggleState, false)
    a160(a200.FPSToggleState,     false)
    a160(a201.AutoRejoinToggleState, false)
